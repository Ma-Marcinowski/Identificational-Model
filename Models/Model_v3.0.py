...
class DataSequence(tf.keras.utils.Sequence):
    ...
    def __init__(self, dataframe, batch_size):
        ...
        self.authors = self.df[self.heads[1:28]].values
    ...
    def get_batch_authors(self, idx):
        batch_authors = self.authors[idx * self.batch_size: (idx + 1) * self.batch_size]
        return np.array(batch_authors)
    ...
    def __getitem__(self, idx):
        ...
        batch_y1 = self.get_batch_authors(idx)
        batch_y2 = self.get_batch_features(idx)
        return ({'input': batch_x}, {'authors': batch_y1, 'features': batch_y2})
...
authors = Dense(27, activation='softmax', name='authors')(auth_drop)
...
model.compile(optimizer=Adam, 
              loss={'authors': 'categorical_crossentropy', 'features': 'binary_crossentropy'}, 
              metrics={'authors': ['categorical_accuracy', tf.keras.metrics.AUC()], 
                       'features': ['binary_accuracy', tf.keras.metrics.AUC()]})

csv_logger = tf.keras.callbacks.CSVLogger('/path/IM_v3.0_Training_Log.csv', separator=',', append=True)

checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='/path/IM_v3.0-{epoch:02d}-{val_authors_categorical_accuracy:.2f}.h5',
                                                monitor='val_authors_categorical_accuracy',
                                                verbose=1,
                                                save_best_only=True,
...
reduceLR = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_authors_loss',
                                               factor=0.1,
                                               patience=5,
                                               verbose=1,
                                               mode='min',
...
#model.save(filepath='/path/IM_v3.0.h5', overwrite=True, include_optimizer=True, save_format='h5')
