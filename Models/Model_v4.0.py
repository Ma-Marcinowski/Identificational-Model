...
class DataSequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_y = self.get_batch_features(idx)
        return ({'input': batch_x}, {'features': batch_y})
...
#model = load_model(filepath='/path/IM_v4.0.h5', compile=True)
...
csv_logger = tf.keras.callbacks.CSVLogger('/path/IM_v4.0_Training_Log.csv', separator=',', append=True)

checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='/path/IM_v4.0-{epoch:02d}-{val_binary_accuracy:.2f}.h5',
...
#model.save(filepath='/path/IM_v4.0.h5', overwrite=True, include_optimizer=True, save_format='h5')
