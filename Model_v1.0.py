...
class DataSequence(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_y = self.get_batch_authors(idx)
        return ({'input': batch_x}, {'authors': batch_y})
...
model = Model(inputs=[input], outputs=[authors])

#model = load_model(filepath='/path/IM_v1.0.h5', compile=True)
...
csv_logger = tf.keras.callbacks.CSVLogger('/path/IM_v1.0_Training_Log.csv', separator=',', append=True)

checkpoint = tf.keras.callbacks.ModelCheckpoint(filepath='/path/IM_v1.0-{epoch:02d}-{val_categorical_accuracy:.2f}.h5',
